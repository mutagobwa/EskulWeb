...
namespace Eskul.Controllers
{
    public class HostelRoomController : Controller
    {
        ...
        public HostelRoomController(IConfiguration configuration, ILoggerErr logger, MyUtilities myUtilities)
        {
        ...
        public async Task<ActionResult> Index(HostelRoom model)
        {
            try
            {
                if (!SessionData.IsSignedIn) { return RedirectToAction("Index", "Login"); }
                ApiResponse response = await _myUtilities.LoadHostelRooms();
                if (response.Success)
                {
                    model.HostelRooms = JsonConvert.DeserializeObject<List<HostelRoom>>(response.PayLoad);
                }
        ...
        // POST: HostelRoomController/Create
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Create(HostelRoom model)
        {
            ...
            try
            {
                ...
                resp = await request.AddAsync<HostelRoom>(model, Url);
        ...
        // GET: HostelRoomController/Delete/5
        public async Task<ActionResult> Delete(int id)
        {
            if (!SessionData.IsSignedIn) { return RedirectToAction("Index", "Login"); }
            Url = $"HostelManagement/HostelRoom/Delete/{SessionData.ClientCode}/{id}";
            try
            {
